       ...
       public async Task<IResult> SubmitQuizResultAsync(CreateQuizResultDTO result, string email)
{
    ...
    return Results.Ok(new { message = "Quiz result submitted", score });
        ...
        {
            var filter = Builders<QuizResult>.Filter.And(
                Builders<QuizResult>.Filter.Gte(r => r.SubmittedAt, startDate),
                Builders<QuizResult>.Filter.Lt(r => r.SubmittedAt, endDate)
            );
            ...
            var users = await _usersRepository.GetByFilterAsync(Builders<User>.Filter.In(u => u.Id, userIds));
            var userDict = users.ToDictionary(u => u.Id, u => u);

            var data = leaderboard.Select(l => new
            {
                User = userDict.ContainsKey(l.UserId) ? new
                {
                    userDict[l.UserId].FirstName,
                    userDict[l.UserId].LastName,
                    userDict[l.UserId].Email,
                    userDict[l.UserId].ProfileImage
                } : null,
                l.AverageScore,
                l.HighestScore,
                l.Attempts
