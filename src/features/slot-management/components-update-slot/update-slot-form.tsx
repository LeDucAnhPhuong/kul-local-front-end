...
import { TimeOnlyInput } from '../components-add-slot/time-only-input';
import { useGetSlotByIdQuery } from '../api.slot';
...
export default function MyForm({ onUpdate }: MyFormProps) {
  ...
  const parseTimeToUTCDate = (timeStr: string): Date => {
  ...
  return new Date(Date.UTC(year, month, date, hour, minute, 0));
};
  ...
  useEffect(() => {
    if (slot) {
      form.reset({
        name: slot.name,
        startTime: parseTimeToUTCDate(slot.startTime),
        endTime: parseTimeToUTCDate(slot.endTime),
      });
    }
  }, [slot]);
