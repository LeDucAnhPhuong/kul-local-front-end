...
function FilterModal<TData, TValue>({ opener, table }: { opener: ReactNode; table: Table<any> }) {
  const [activeFilter, setActiveFilter] = useState<
    ColumnDefTemplate<HeaderContext<any, unknown>> | undefined | null
  >(null);
  const [currentColumn, setCurrentColumn] = useState<Column<TData, TValue> | null>(null);
  const [currentHeader, setHeader] = useState<Header<TData, TValue> | null>(null);
                    ...
                    return (
                      header.column.getCanFilter() && (
                        ...
                        >
                          ...
                          {header.column.getFilterValue() && <Badge>Thiếp lập</Badge>}
                        </Button>
                      )
                    );
                  }),
...
const FilterItem = <TData, TValue>({
  ...
  const headerfn = flexRender(header.column.columnDef.header, header.getContext());
  ...
  switch (filterVariant) {
      ...
      return (
        <>
          ...
          <Select
            ...
            onValueChange={(e: string) => column.setFilterValue(e === 'none' ? undefined : e)}
          >
            <SelectTrigger>
    ...
    case 'numberRange':
      const values: Array<number> = table.getRowModel()?.rows.map((row) => row.getValue(column.id));
      ...
      return (
        <>
          ...
          <Input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => column.setFilterValue(e.target.value)}
          />
        </>
      );
  }
