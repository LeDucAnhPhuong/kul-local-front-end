        ...
        <div className="flex items-center justify-center">
          ...
          >
            <DateTimeLocalInput
              ...
              disabledDateTimeTo={disabledDateTimeTo}
              disabledDateTimeFrom={disabledDateTimeFrom}
            />
  ...
  return (
    <div className="space-y-2 pr-3 py-3 relative">
      ...
      <ScrollArea
        ...
        className="h-[90%] w-full px-2 focus-visible:outline-0 focus-visible:ring-0 focus-visible:ring-offset-0 focus-visible:border-0 py-0.5"
        style={{
          height,
        }}
      >
...
>(({ placeholder, ...props }, ref) => {
  ...
  React.useEffect(() => {
    ...
    const timeVal = `${hour >= 12 ? hour % 12 : hour}:${new Date().getMinutes()} ${
      hour >= 12 ? 'PM' : 'AM'
    }`;
...
const DateTimeLocalInput = ({
  ...
  const formateSelectedDate = React.useCallback(
    ...
    [value, Time, disabledDateTimeFrom, disabledDateTimeTo],
  );

  const isDateDisabled = (date: Date): boolean => {
    if (typeof disabled === 'function' && disabled(date)) return true;

    if (disabledDateTimeTo && date > new Date(disabledDateTimeTo)) return true;
    if (disabledDateTimeFrom && date < new Date(disabledDateTimeFrom)) return true;
