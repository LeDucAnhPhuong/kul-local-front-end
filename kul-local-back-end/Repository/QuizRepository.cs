...
namespace kul_local_back_end.Repositories
{
    public class QuizRepository : BaseRepository<Quiz>, IQuizRepository
    {
        ...
        public async Task<IResult> GetAvailableQuizzesForStudentAsync(string email)
        {
            ...
            // Step 3: Lấy tất cả bài đã nộp của học sinh
            ...
            // Step 4: Gắn trạng thái
            var resultData = quizzes.Select(q => new
            {
                q.Id,
                q.Title,
                q.Date,
                ...
                q.IsPublic,
                q.CreatedBy,
                status = submittedQuizIds.Contains(q.Id) ? "done" : "not_yet",
            });

            return Results.Ok(new { data = resultData });
        }
    }
}
